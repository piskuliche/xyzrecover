...
from .core import PerceptionConfig, recover_xyz_file, records_to_json, write_csv, write_sdf
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--total-charge", type=int, default=None, help="Known total charge for each XYZ block")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Comma-separated charge states to try when charge is unknown",
    )
    parser.add_argument("--no-split", action="store_true", help="Do not split disconnected covalent components")
    parser.add_argument("--use-hueckel", action="store_true", help="Use extended Hueckel connectivity if RDKit supports it")
    parser.add_argument("--use-vdw", action="store_true", help="Use RDKit van der Waals connectivity")
    parser.add_argument("--cov-factor", type=float, default=1.30, help="Covalent-radius multiplier for connectivity")
    parser.add_argument(
        "--radicals-instead-of-charges",
        action="store_true",
        help="Tell RDKit to use radicals instead of charged fragments when assigning valence",
    )
    parser.add_argument("--no-chiral", action="store_true", help="Do not embed 3D-derived chirality")
    parser.add_argument("--keep-explicit-h-smiles", action="store_true", help="Use explicit-H SMILES as the primary SMILES")
    parser.add_argument("--max-iterations", type=int, default=0, help="RDKit DetermineBonds maxIterations; 0 means no cap")
...
def main(argv: list[str] | None = None) -> int:
    ...
    config = PerceptionConfig(
        ...
        candidate_charges=args.candidate_charges,
